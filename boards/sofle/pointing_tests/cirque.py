...
class AbsoluteHandler:
    cfg = (
        ...
        0x00,
        _REG_FEED_CFG1,
        _FEED1_ENABLE | _FEED1_NO_FILTER | _FEED1_ABSOLUTE,
    )
    x_old = 0
    y_old = 0
    ...
    def handle(buffer: bytearray, keyboard) -> None:
        ...
        if x_pos != 0 and y_pos != 0 and AbsoluteHandler.x_old != 0 and AbsoluteHandler.y_old != 0:
            dx = AbsoluteHandler.x_old - x_pos
            dy = AbsoluteHandler.y_old - y_pos
            debug(
                ' dx:', dx,
                ' dy:', dy
            )
        ...
        AbsoluteHandler.x_old = x_pos
        AbsoluteHandler.y_old = y_pos
...
class GlidePoint(Module):
    ...
    def _check_firmware(self) -> bool:
        ...
        self._read(2)
        print("_check_firmware", self._buffer[:2])
